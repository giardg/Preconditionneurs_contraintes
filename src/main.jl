...
M = M #M = M'*M pour hermitien, mais pas nécessaire (pour gmres oui)
N = N'*N;
...
for k = 1:kmax
    t1 = @elapsed x, stats = solvePrecond(M,A,N,b, "cgs", false, "I", k);
    res1[k] = norm(b-mat*x)
    time1[k] = t1
    t2 = @elapsed x2, stats = solvePrecond(M,A,N,b, "cgs", true, "I", k);
    res2[k] = norm(b-mat*x2)
    time2[k] = t2
    t3 = @elapsed x3, stats = solvePrecond(M,A,N,b, "cgs", true, "Diagonal", k);
    res3[k] = norm(b-mat*x3)
    time3[k] = t3
    t4 = @elapsed x4, stats = solvePrecond(M,A,N,b, "cgs", true, "LLDL", k);
