...
x1, stats = solvePrecond(M,A,N,D, "gmres", false, "I", "G", 1e-8, 1e-8, m+n);
...
x2, stats = solvePrecond(M,A,N,D, "gmres", true, "I", "G", 1e-8, 1e-8, m+n);
println(matrix_name, " -- ", " G = I Nombre d'itérations: ", length(stats.residuals)-1)
println(matrix_name, " -- ", " G = I Résidu: ", norm(mat*x2-D))

x3, stats = solvePrecond(M,A,N,D, "gmres", true, "Diagonal", "G", 1e-8, 1e-8, m+n);
